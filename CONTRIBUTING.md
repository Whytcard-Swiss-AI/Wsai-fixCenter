...
4. Run `python -m wsai_fckdot self-test` and the full test suite.
...
A setup adapter lives in `wsai_fckdot.setup_manager` and must use a fixed workspace-relative path, carry a generated ownership marker, contain no credential values, preserve user-owned files, and include create/update/conflict, profile-switching and cross-platform tests. Never add an adapter that writes directly to a user home directory.
